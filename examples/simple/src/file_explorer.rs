...
async fn init() {
    ...
    let game = px::EngineWrapper::new("Path".to_owned(), (500, 500, 1)).await;
    let mut pw = PathView::new(PathBuf::from("."));
    use px::inputs::Keycodes::{Down, Escape, Left, Right, Up};
    game.run(move |game: &mut px::Engine| {
        game.clear([0x00, 0x00, 0x00].into());
        ...
        game.draw_text((0, 0), 2, [1.0, 1.0, 1.0].into(), &current_filename);
